...
class DataDict:
    ...
    def __repr__(self) -> str:
        return f"Robot with Sensors: {list(self.data.keys())}"
    ...
    def by_stamps(self, stamps):
        ...
        # all keys except mocap
        sensors = list(self.data.keys() - ["mocap"])
        indices_dict = {}

        for sensor in sensors:
            indices = []
            for s in stamps:
                index = self.get_index(sensor, s)
        ...
        for sensor in sensors:
            out[sensor] = out[sensor].loc[indices_dict[sensor]]
        return out
    
    def get_index(self, topic: str, stamp: float) -> int:
        """
        ...
        """
        mask = self.data[topic]["timestamp"] <= stamp
